import-module au
$releases = 'https://www.nirsoft.net/utils/alternate_data_streams.html'
...
function global:au_GetLatest {
    ...
    $version = [regex]::Match($DownloadPage.RawContent,'(?<=AlternateStreamView\s*v\s*)\d+[.\d]+').Value
    
    # unversioned url 
    $url32 = $downloadPage.links `
    | ForEach-Object { $_.href } `
    | Where-Object { $_ -match 'alternatestreamview\.zip' } `
    | Select-Object -First 1

    $url64 = $downloadPage.links `
    | ForEach-Object { $_.href } `
    | Where-Object { $_ -match 'alternatestreamview-x64\.zip' } `
    | Select-Object -First 1
